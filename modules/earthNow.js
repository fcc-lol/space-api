...
const findMostRecentAvailableDate = async (requestedDate, variant = 'natural', maxDaysBack = 30) => {
...
export const getEarthImageryMetadata = async (date = 'latest', variant = 'natural') => {
    try {
        ...
        if (!Array.isArray(data) || data.length === 0) {
            console.log(`No images found for requested date ${date}, searching for most recent available date...`);
...
export const getEarthImageURL = async (date = 'latest', variant = 'natural', index = 0) => {
...
export const getEarthImage = async (date = 'latest', variant = 'natural', index = 0) => {
